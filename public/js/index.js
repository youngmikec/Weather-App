const countries = [
  ...
  class WeatherApp{
    ...
    drawHumidityCircle(obj){
      let parent = document.querySelector('#humidity');
    ...
    drawPressureCircle(obj){
      let parent = document.querySelector('#wind-speed');
      let canvas = document.createElement('canvas');
      canvas.setAttribute('width', '100%');
      canvas.setAttribute('height', '100%');
      parent.appendChild(canvas);
    ...
    removeCanvas(){
      let parent1 = document.querySelector('#humidity');
      let parent2 = document.querySelector('#wind-speed');
      parent1.removeChild(parent1.lastElementChild);
      parent2.removeChild(parent2.lastElementChild);
    }
    removeTemperature(){
    ...
    removeHistory(){
      let top = document.querySelector('#top-component');
      top.innerHTML = '';
  
    }
    populateHistory(obj){
      let date = new Date(obj.dt);
      let da = date.getDate();
      let d = date.getDay();
      if(d == 0){
        d = 'Sun'
      ...
      container.setAttribute('class', 'forecast-card');
      icon.setAttribute('class', 'fa fa-cloud');
      icon.setAttribute('aria-hidden', 'true');
      heading2.style.fontSize = "40px";
      heading2.appendChild(icon);
      heading1.textContent = `${d} ${da}`;
      heading4.textContent = obj.weather[0].main;
      heading3.innerHTML = Math.ceil(obj.temp.day - 273);
      sup1.textContent = 'o';
      heading3.appendChild(sup1);
      span.textContent = Math.ceil(obj.feels_like.day - 273);
      sup2.textContent = 'o';
      span.appendChild(sup2);
    ...
    drawLineChart(obj){
        let dobj = obj;
        let array = [['time', 'temp']];
          for(let d = 0; d < dobj.daily.length; d++){
            let date = new Date(dobj.daily[d].dt);
            let time = date.getMinutes();
            let tem = dobj.daily[d].temp.day - 273;
            let sub = [time, tem];
            array.push(sub);
          }
